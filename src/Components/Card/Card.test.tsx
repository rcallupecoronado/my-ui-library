...
}> = ({ title, content, children, testBackgroundColor, testDisabled, testTextColor, ...rest }) => {
    const filteredProps = omit(rest, ['testBackgroundColor', 'testDisabled', 'testTextColor']);
    return (
        <Card
            title={title}
            content={content}
...
describe("Card Component", () => {
    ...
    test("applies custom background color when provided", () => {
      render(<TestWrapper title="Test Title" content="Test Content" testBackgroundColor="lightblue" />);
  ...
  test("checks that the component is disabled", () => {
      render(<TestWrapper title="Test Title" content="Test Content" testDisabled={true} />);
      const cardElement = screen.getByRole("region");
  ...
  test("applies custom text color when provided", () => {
      render(<TestWrapper title="Test Title" content="Test Content" testTextColor="darkblue" />);
      const cardElement = screen.getByRole("region");
